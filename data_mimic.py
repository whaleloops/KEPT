...
class MimicFullDataset(Dataset):
    def __init__(self, version, mode, truncate_length, tokenizer,
        ...
        if term_count == 1:
            ...
            c_desc_list = []
            with open(f'/home/zhichaoyang/mimic3/ICD-MSMN/embedding/icd_mimic3_random_sort.json', 'r') as f: #TODO: change path
            ...
            c_desc_list = np.array(c_desc_list).swapaxes(1,0)

        max_pronpt_length = 0
        for groupa in c_desc_list:
            assert len(groupa) == 50
            descriptions = " " + " <mask>, ".join(groupa) + " <mask>. "
            tmp = self.tokenizer.tokenize(descriptions)
            max_pronpt_length = max(max_pronpt_length, len(tmp))
        
        self.global_window = max_pronpt_length + 1 # 500
        ...
        tmp_df = []
        for index in range(self.len):
            for descriptions in c_desc_list:
                descriptions = " " + " <mask>, ".join(descriptions) + " <mask>. "
                text = self.df[index]['TEXT']
                text = re.sub(r'\[\*\*[^\]]*\*\*\]', '', text)  # remove any mimic special token like [**2120-2-28**] or [**Hospital1 3278**]
                ...
                if len(tmp) <= self.truncate_length:
                    num_pro_token.append(len(tmp))
                    result = {}
                    result['TEXT'] = descriptions + re.sub(r'  +', ' ', text.lower().replace("\n"," "))
                    result['LABELS'] = self.df[index]['LABELS']
                    result['hadm_id'] = self.df[index]['hadm_id']
...
def my_collate_fn(features: List[InputDataClass]) -> Dict[str, torch.Tensor]:
    ...
    global_attention_mask[:,0:500] = 1 # TODO change 408 to self.global_window
